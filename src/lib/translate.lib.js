...
export default async function translate_lib(text, target) {
  if (!_isSupported(target))
    return new Error(`Language "${target}" is not supported!`);
  ...
  if (db && db[target]) {
    ...
    const response = await _translate(text, target)
    const translation = escape(response);
    if (!translation)
      return new Error(`No translation found for language "${target}"`);
